    ...
    while Q:
        ...
        for e in out_edges.get(L.node, []):
            ...
            if any(d.cost<=L2.cost and d.reward>=L2.reward and d.hops<=L2.hops for d in doms):
                continue
            new = [d for d in doms if not (L2.cost<=d.cost and L2.reward>=d.reward and L2.hops<=d.hops)]
            new.append(L2)
            labels[e.dst] = new
            Q.append(L2)
    ...
    for _ in range(N):
        ...
        for e in nbrs:
            ...
            if triple in E_used:
                ...
                c=cost_fn(triple); r=edge_ic.get(triple,0.0)-alpha
            if total_cost+c> C_rem: continue
