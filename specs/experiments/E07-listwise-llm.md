# E07 — Direct and comparative LLM judgment with evidence acquisition
...
**Already implemented:** Listwise prompt, categorical probability, order, and aggregation primitives exist; scorer grouped execution still rejects listwise modes.

**Agent must implement:** OpenRouter grouped runtime, source-level eligibility, score-blind direct packets, no-summary comparison, bounded evidence requests, complete response/cost ledger, and source-level integration.

**Inputs/bindings to resolve:** Pin one OpenRouter model/provider supporting the required decision output/logprob contract; reserve token/spend budget.
...
Primary focused case: **D0 or a feature-selected hard/NIL case; maximum 200 forced-screen sources**.
Resource envelope: **llm** in RUN-PLAN.
Prerequisites/consumed outputs: **E00, pool_freeze, E25_initial**. A pool-freeze or selected-head dependency
...
Force identical development source populations for judgment comparisons; then evaluate deployable source routing. Do not require two pair uncertainty gates. Cap candidates at five, retain omitted ranks and categorical vector, and refit only allowed calibration. Gate quality, judge quality, and trust weight are separate factors, not one confounded Cartesian sweep. Explicitly distinguish equivalence from broader/narrower, related and unresolved concepts; retain cited evidence IDs and qualifier differences in the judgment record. Compare the 64-token and 256-token brief controls with the same direct-packet facts, reporting actual total tokens rather than equating output ceilings with cost.

At most **7 distinct treatment configurations/cells as specified below** before any explicitly declared source expansion. This is a bounded sequential design, not a Cartesian product. Shared deterministic controls are computed once.

- brief_binary: current compressed-brief binary control.
- brief_256_binary: same source facts and brief schema with a 256-token generation ceiling.
- packet_binary: same candidates judged from the direct scored packet.
- facts_binary: score-blind structured ontology evidence.
- facts_listwise: same facts/candidates, one comparative call including none.
- retrieved_listwise: at most two extra evidence packets then one judgment.
- second_model: extended-only sensitivity of the frozen comparison.
...
Primary outcome/guard: **Correction-minus-harm and final F1 at matched cost; list-length/order/NIL slices mandatory.**
...
- Binary and listwise controls receive the same candidate identities and evidence content.
- Raw categorical probabilities reorder back to canonical targets correctly.
- High-confidence close competitors can enter source-level routing.
- Interrupted OpenRouter work reuses completed requests with no local generative fallback.
- Evidence retrieval accesses ontology facts only and cannot add reporting gold or alter the G1 pool.

Durable boundaries: **Source request plans, fetched evidence, raw responses, parsing and calibrated decisions.**
