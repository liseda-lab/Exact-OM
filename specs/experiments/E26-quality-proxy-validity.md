# E26 — Quality validity, inactivity, and sharpening
...
**Already implemented:** Five sigma decomposition modes and lexical quality variants exist; component dumps and fitted reliability path remain incomplete.

**Agent must implement:** Complete quality-component artifacts, duplicate/singleton/count-aware checks, candidate-level ambiguity quality, controlled entropy temperature and conditional reliability fitting.

**Inputs/bindings to resolve:** Resolve existing encoder revisions and usable training groups; no separate dataset needs to be invented.
...
Primary focused case: **D0; D1 for a populated structural channel**.
Resource envelope: **channels** in RUN-PLAN.
Prerequisites/consumed outputs: **E00, baseline_evidence**. A pool-freeze or selected-head dependency
...
Run decomposition before E19. Compare the old q=1 singleton against duplicate/near-duplicate synonyms without inventing a single-label default: all_labels is currently true. Separate active-channel suppression, confidence sharpening and q. Test lexical-only and lexical-plus-attributes controls from shared evidence. Brier is a proper score, but quality-bin Brier alone is not proof of calibration.

At most **9 distinct treatment configurations/cells as specified below** before any explicitly declared source expansion. This is a bounded sequential design, not a Cartesian product. Shared deterministic controls are computed once.

- decomposition: full, constant_q, no_sharpening, suppression_only, uniform; five shared-evidence arms.
- candidate_margin: target-concept ambiguity quality on the selected decomposition.
- entropy_count_control: frozen temperature and explicit singleton/count behavior.
- encoder_agreement: inspect best-target agreement separately from score similarity.
- reliability_head: one optional source-group-supervised conditional-error head.
...
Primary outcome/guard: **Mechanism decomposition and final F1; null/removal is a valid outcome.**
...
- Duplicating synonymous representations does not silently create extra support or unjustified authority loss in the candidate variant.
- Quality dumps retain components, label counts, inactive flags and enough top-m information for replay.
- Supervised reliability is never presented as label-free on the target pair.
- All decomposition arms share the same evidence; no exhaustive quality-by-fusion factorial.

Durable boundaries: **Raw quality components, normalized label/candidate statistics, replayed fusion or reliability fit.**
